...
DEFAULT_INPUT_SIZES = ("3x224x224", "3x448x448", "3x672x672")
MAXVIT_OUTPUT_STRIDE = 32
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Microbenchmark MaxViT FlashInfer NVFP4 Linear layers and primitive costs."
    )
    parser.add_argument("--variant", choices=MAXVIT_VARIANT_CHOICES, default="tiny")
    parser.add_argument("--model-path", default=None)
    parser.add_argument("--batch-size", type=int, default=1)
    parser.add_argument("--batch-sizes", type=int, nargs="+", default=None)
    parser.add_argument("--input-sizes", nargs="+", default=None)
...
def main() -> None:
    ...
    output = args.output or f"artifacts/results/{variant_info.result_key}_nvfp4/microbench.csv"
    ...
    model, _, report = load_maxvit_flashinfer_nvfp4(
        args.model_path,
        dtype=args.dtype,
        device=device,
    ...
    print(
        "maxvit nvfp4 microbench: "
        f"variant={args.variant} layers={len(modules)} replaced={report.replaced_linear_count} "
        f"skipped={report.skipped_linear_count} output={output}"
    )

    input_size_divisor = _maxvit_input_size_divisor(model)
    raw_input_sizes = args.input_sizes or _default_input_sizes(input_size_divisor)
    input_sizes = [_parse_input_size(value) for value in raw_input_sizes]
    for batch_size in batch_sizes:
        for input_size in input_sizes:
            ...
            try:
                _validate_maxvit_input_size(input_size, input_size_divisor)
                ...
                print(
                    ...
                    f"layer_calls={len(calls)}"
                )
            except Exception as exc:
                _write_error_row(output, base, exc)
                _cleanup_cuda()
...
def _default_input_sizes(input_size_divisor: int) -> tuple[str, ...]:
    if input_size_divisor >= 512:
        return (f"3x{input_size_divisor}x{input_size_divisor}",)
    return DEFAULT_INPUT_SIZES


def _maxvit_input_size_divisor(model: torch.nn.Module) -> int:
    partition_sizes: list[int] = []
    for module in model.modules():
        partition_size = getattr(module, "partition_size", None)
        ...
        if isinstance(partition_size, int):
            ...
            continue
        try:
            partition_sizes.extend(int(value) for value in partition_size)
        except TypeError:
            continue
    if not partition_sizes:
        return 224
    return MAXVIT_OUTPUT_STRIDE * max(partition_sizes)


def _validate_maxvit_input_size(input_size: tuple[int, int, int], input_size_divisor: int) -> None:
    channels, height, width = input_size
    if channels != 3:
        raise ValueError(f"MaxViT expects 3 input channels, got input_size={_shape_str(input_size)}")
    if height != width:
        raise ValueError(f"This MaxViT microbenchmark expects square inputs, got input_size={_shape_str(input_size)}")
    if height % input_size_divisor != 0:
        examples = ", ".join(
            f"3x{input_size_divisor * factor}x{input_size_divisor * factor}" for factor in range(1, 4)
        )
        raise ValueError(
            "MaxViT window partition requires this benchmark input height/width to be a multiple of "
            f"{input_size_divisor}; got input_size={_shape_str(input_size)}. Try {examples}."
        )
