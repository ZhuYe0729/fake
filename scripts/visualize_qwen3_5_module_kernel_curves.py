#!/usr/bin/env python
"""Plot Qwen3.5 module-forward kernel latency curves.

Each subplot is one Qwen3.5 compressible Linear shape. Curves show packaged
...
DEFAULT_INPUT = Path("artifacts/results/benchmarks/module/Qwen3.5-2B/kernel/qwen35_2b_module_kernel_curves.csv")
DEFAULT_OUTPUT = Path("artifacts/results/benchmarks/module/Qwen3.5-2B/kernel/qwen35_2b_module_kernel_latency_curves.png")
...
def _plot_latency_curves(rows: list[dict[str, str]], output: Path, *, title: str) -> None:
    ...
    if not groups:
        raise RuntimeError("No Qwen3.5 linear groups found in CSV")
...
def main() -> None:
    ...
    model = rows[0].get("model", "Qwen3.5") if rows else "Qwen3.5"
    title = args.title or f"{model} Packaged Linear.forward Latency Curves by Shape"
    _plot_latency_curves(rows, args.output, title=title)
