...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate DINOv3 ViT-7B/16 CUTLASS sparse BF16 linear classifier.")
    ...
    parser.add_argument("--output", default="artifacts/results/dinov3_vit7b16_cutlass_sparse_bf16/accuracy.csv")
...
def main() -> None:
    ...
    if args.runtime_checkpoint:
        ...
        if runtime_load.metadata.get("backend") != "sparse_bf16":
            raise ValueError(f"Expected sparse_bf16 runtime checkpoint, got {runtime_load.metadata.get('backend')}")
        checkpoint_metadata = runtime_load.metadata.get("source_checkpoint_metadata", {})
    else:
        model, config, report, checkpoint_metadata = load_dinov3_vit7b16_cutlass_sparse_bf16_classifier(
            backbone_path=args.backbone_path,
            head_path=args.head_path,
            device=device,
    ...
    print(
        "dinov3 cutlass sparse bf16 accuracy done: "
        f"top1={row['top1']} top5={row['top5']} samples={result.num_samples} "
        f"replaced={report.replaced_linear_count} skipped={report.skipped_linear_count} output={args.output}"
    )
    if report.skipped:
