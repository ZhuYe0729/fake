#!/bin/bash
#SBATCH --job-name=dinov3_cutlass_nvfp4_acc
#SBATCH --partition=gpu_5090
#SBATCH --gpus=1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --output=out/dinov3_cutlass_nvfp4_acc_%j.out
#SBATCH --error=err/dinov3_cutlass_nvfp4_acc_%j.err
...
mkdir -p out err artifacts/results/dinov3_vit7b16_cutlass_nvfp4

BATCH_SIZE="${BATCH_SIZE:-1}"
NUM_WORKERS="${NUM_WORKERS:-4}"
RESIZE_SIZE="${RESIZE_SIZE:-256}"
LOG_INTERVAL="${LOG_INTERVAL:-50}"
OUTPUT="${OUTPUT:-artifacts/results/dinov3_vit7b16_cutlass_nvfp4/accuracy.csv}"
...
PYTHONPATH=. python scripts/eval_dinov3_vit7b16_cutlass_nvfp4_accuracy.py \
