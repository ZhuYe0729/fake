...
from fake.kernels.cutlass_sparse_nvfp4 import CutlassSparseNVFP4Config, count_cutlass_sparse_nvfp4_modules
...
from fake.models.maxvit_cutlass_checkpoint import load_maxvit_cutlass_sparse_storage
from fake.models.maxvit_cutlass_sparse_nvfp4 import load_maxvit_cutlass_sparse_nvfp4
from fake.utils.csv_io import append_csv_row


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate MaxViT CUTLASS sparse NVFP4 on ImageNet val zip dataset.")
...
def main() -> None:
    ...
    output = args.output or f"artifacts/results/{variant_info.result_key}_cutlass_sparse_nvfp4/accuracy.csv"
    runtime_load = None
    if args.storage_checkpoint:
        model, config, report, runtime_load = load_maxvit_cutlass_sparse_storage(
        ...
        model, config, report, checkpoint_metadata = load_maxvit_cutlass_sparse_nvfp4(
            model_path=args.model_path,
            device=device,
            variant=args.variant,
    ...
    print(
        "maxvit cutlass sparse nvfp4 accuracy done: "
