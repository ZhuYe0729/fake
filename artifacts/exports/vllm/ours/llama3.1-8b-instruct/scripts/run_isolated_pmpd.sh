...
SCENARIO="${SCENARIO:?set SCENARIO}"; DATASET="${DATASET:?set DATASET}"
QUESTION_BEGIN="${QUESTION_BEGIN:-0}"; QUESTION_END="${QUESTION_END:?set QUESTION_END}"
BATCH_SIZE="${BATCH_SIZE:-4}"; GPU_MEMORY_UTILIZATION="${GPU_MEMORY_UTILIZATION:-0.75}"
IWSLT_FILTER_TOKENIZER="${IWSLT_FILTER_TOKENIZER:-/home/agent/wja/data/models/shakechen/Llama-2-7b-chat-hf}"
OUT_DIR="${OUT_DIR:-${ROOT}/max_speed/${SCENARIO}/results/quality}"; LABEL="${LABEL:-ours_max_speed_${SCENARIO}}"; SKIP_FINAL_METRICS="${SKIP_FINAL_METRICS:-0}"
...
for ((begin=QUESTION_BEGIN; begin<QUESTION_END; begin+=BATCH_SIZE)); do
  ...
  extra=(); (( begin > QUESTION_BEGIN )) && extra=(--append)
  "${PYTHON_BIN}" "${VLLM_ROOT}/artifacts/dev/011_phase_switch_linear_test/pmpd_vllm_eval.py" \
    ...
    --model-path "${ROOT}/max_speed/${SCENARIO}/checkpoint" --model-id "${LABEL}" --output-dir "${OUT_DIR}" \
    ...
    --iwslt-filter-tokenizer "${IWSLT_FILTER_TOKENIZER}" --skip-metrics "${extra[@]}" > "${OUT_DIR}/logs/${DATASET}/${begin}_${end}.log" 2>&1
done
if [[ "${SKIP_FINAL_METRICS}" != 1 ]]; then
  "${PYTHON_BIN}" "${ROOT}/../../../../../references/pmpd_eval_kit/pmpd_eval.py" --dataset "${DATASET}" --metrics-only "${OUT_DIR}/${DATASET}/${LABEL}-fp16.jsonl" > "${OUT_DIR}/logs/${DATASET}/metrics.log" 2>&1
fi
