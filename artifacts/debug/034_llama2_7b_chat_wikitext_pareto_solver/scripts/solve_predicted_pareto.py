...
CUTLASS=ROOT.parent/'cutlass_wrapper'
...
def parse():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--scenario',choices=tuple(SCENARIO),required=True);p.add_argument('--output-root',type=Path,default=Path(__file__).resolve().parents[1]);p.add_argument('--source-root',type=Path,default=SOURCE);p.add_argument('--predictor-root',type=Path,default=DEFAULT_MODEL_ROOT);p.add_argument('--phase-local-errors',action='store_true',help='read prefill/decode local errors from source-root/local_errors');p.add_argument('--budget-bins',type=int,default=1600);p.add_argument('--points',type=int,default=17);return p.parse_args()
def read(p):
 with p.open(newline='') as f:return list(csv.DictReader(f))
def fused_error(source,phase_local):
 if phase_local:
  out={};methods=[m for m in METHODS if m!='dense_bf16']
  for phase in ('prefill','decode'):
   for method in methods:
    for row in read(source/'local_errors'/f'{phase}_{method}.csv'):
     out[phase,int(row['layer_bucket']),row['fused_type'],method]=float(row['output_rel_mse'])
   for bucket in range(4):
    for typ in TYPES:out[phase,bucket,typ,'dense_bf16']=0.
  return out
 ...
 return out
def feature(policy,phase,err,phase_local):
 x=torch.zeros((5,4,4),dtype=torch.float64)
 for name,e in policy['method_map'].items():
  layer=int(name.split('.')[2]);typ=name.rsplit('.',1)[-1];m=e[f'{phase}_method'];value=err[phase,layer//8,typ,m] if phase_local else err[layer,typ,m];x[METHODS.index(m),layer//8,TYPES.index(typ)]+=value
 return x
def resolve_policy_path(source,scenario,entry):
 """Use a manifest path when valid, otherwise relocate its basename locally."""
 recorded=Path(entry['path'])
 if recorded.is_file():return recorded
 relocated=source/'policies'/scenario/recorded.name
 if relocated.is_file():return relocated
 raise FileNotFoundError(f"policy {entry['policy_id']} is unavailable: {recorded} or {relocated}")
def fit_proxy(source,scenario,err,phase_local):
 manifest=json.loads((source/'policies'/scenario/'manifest.json').read_text());pol={r['policy_id']:json.loads(resolve_policy_path(source,scenario,r).read_text()) for r in manifest};nll={r['policy_id']:r for r in read(source/'nll'/f'{scenario}.csv')};train=torch.tensor([r['split']=='train' for r in manifest]);xp=torch.stack([feature(pol[r['policy_id']],'prefill',err,phase_local) for r in manifest]);xd=torch.stack([feature(pol[r['policy_id']],'decode',err,phase_local) for r in manifest]);y=torch.tensor([float(nll[r['policy_id']]['delta_prefill_nll'])+(80*float(nll[r['policy_id']]['delta_decode_nll']) if scenario=='prefill_decode' else 0.) for r in manifest],dtype=torch.float64);X=xp+(80*xd if scenario=='prefill_decode' else 0.);scale=X[train].sum((1,2,3)).mean().clamp(min=1e-12);X=X/scale
...
def latency_tables(predictor,scenario):
 ...
 return out
def candidates(predictor,scenario,fit,err,phase_local):
 lat=latency_tables(predictor,scenario);coef=torch.tensor(fit['coef'],dtype=torch.float64);out=[];steps=SCENARIO[scenario][2]
 for index,(name,layer,typ) in enumerate(modules()):
  ...
  for pm,dm in legal_pairs(scenario):
   ...
   pre_error=err['prefill',layer//8,typ,pm] if phase_local else err[layer,typ,pm];decode_error=err['decode',layer//8,typ,dm] if phase_local else err[layer,typ,dm]
   q=(pre_error+(80*decode_error if steps else 0.))*float(coef[METHODS.index(pm),layer//8,TYPES.index(typ)]) / fit['scale']
...
def main():
 a=parse();outroot=a.output_root/a.scenario;err=fused_error(a.source_root,a.phase_local_errors);fit=fit_proxy(a.source_root,a.scenario,err,a.phase_local_errors);predictor=KernelLatencyPredictor(model_root=a.predictor_root,kernels=('dense_bf16','dense_nvfp4','sparse_bf16','sparse_nvfp4','marlin_nvfp4'));groups=candidates(predictor,a.scenario,fit,err,a.phase_local_errors);maxq=sum(max(r['quality_cost'] for r in g) for g in groups);ratios=[0.]+[10**(-3+i*(3/max(a.points-2,1))) for i in range(a.points-1)];points=[];seen=set()
 ...
 dense=next(p['raw_predicted_linear_ms'] for p in points if p['quality_ratio']==0.0);[p.update({'raw_linear_speedup_vs_dense':dense/p['raw_predicted_linear_ms']}) for p in points];write_csv(outroot/'pareto/pareto_points.csv',points);(outroot/'frozen_proxy.json').write_text(json.dumps({'scenario':a.scenario,**fit,'phase_local_errors':a.phase_local_errors,'quality_intercept_excluded_from_optimizer':True},indent=2)+'\n');(outroot/'pareto/metadata.json').write_text(json.dumps({'scenario':a.scenario,'modules':len(groups),'candidate_actions':sum(map(len,groups)),'budget_bins':a.budget_bins,'points':len(points),'predicted_only':True,'predictor_root':str(a.predictor_root),'phase_local_errors':a.phase_local_errors,'speed_metric':'raw KernelLatencyPredictor linear latency'},indent=2)+'\n');print(f'wrote {len(points)} predicted points')
