#!/usr/bin/env bash
# One PMPD shard on an explicitly selected physical GPU.
set -euo pipefail
...
PHYSICAL_GPU="${PHYSICAL_GPU:?physical GPU required}"
...
"${PYTHON_BIN}" "${ROOT}/scripts/run_pmpd_on_gpu.py" --physical-gpu "${PHYSICAL_GPU}" \
  --script "${VLLM_ROOT}/artifacts/dev/011_phase_switch_linear_test/pmpd_vllm_eval.py" \
