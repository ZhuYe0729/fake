...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run prefill-only WikiText-2 loss ablations by layer/type/layer-type.")
...
def main() -> None:
    ...
    output_csv = args.output_csv or args.output_root / "ablations" / f"loss_ablation_{'_'.join(methods)}.csv"
    ...
    for method in methods:
        for policy in policies:
            ...
            if not selected:
                continue
            print(f"running method={method} policy={policy['policy']} selected={len(selected)}")
    ...
    write_json(
        ...
        loss_ablation_metadata(args, calib_metadata, methods, policies),
    )
