...
def main() -> None:
    ...
    policies: dict[str, Path] = {f"uniform_p{index:02d}": RUN / f"policies/prefill_only/p{index:02d}.json" for index in range(5)}
    with (RUN / "pareto/pareto_points.csv").open(newline="") as handle:
        for row in csv.DictReader(handle):
            policies[f"point_{int(row['point_index']):03d}"] = Path(row["policy_json"])
