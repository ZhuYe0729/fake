#!/usr/bin/env python3
"""Fail-fast environment and dependency audit for the isolated 067 run."""
...
from common import (BUNDLE, COSPAQ_PYTHON, CUTLASS, EXPORTER, MODEL, REPO,
...
def main() -> None:
    ...
    expected_model = {"model_type": "llama", "num_hidden_layers": 32, "hidden_size": 4096,
    ...
    if any(model_config.get(key) != value for key, value in expected_model.items()):
        errors.append("model is not the expected 32-layer Llama3.1-8B GQA architecture")
    runtime_hits = {}
    ...
    for role, python, modules in (("cospaq", COSPAQ_PYTHON, "torch,transformers,datasets,numpy,pandas"),
                                  ("vllm", VLLM_PYTHON, "torch,transformers,vllm,lm_eval")):
