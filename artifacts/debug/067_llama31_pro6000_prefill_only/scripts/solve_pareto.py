#!/usr/bin/env python3
"""Solve Llama3.1 prefill-only Pareto policies using the real-vLLM NLL proxy."""
...
SHAPES = {"qkv_proj": (6144, 4096), "o_proj": (4096, 4096), "gate_up_proj": (28672, 4096), "down_proj": (4096, 14336)}
...
def main() -> None:
    ...
    (root / "pareto/frozen_quality_model.json").write_text(json.dumps({"label_backend": "real-vLLM prompt-logprob", "quality_model": str(root / "reports/quality/model.json"), "quality_intercept_excluded_from_optimizer": True, "quality_metric": "predicted delta NLL vs dense BF16", "speed_model": "067 Llama3.1 Pro 6000 KernelLatencyPredictor module-forward only", "one_time_weight_conversion_excluded": True, "predictor_root": str(args.predictor_root.resolve()), "points": len(points)}, indent=2) + "\n")
