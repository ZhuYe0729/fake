...
def main() -> None:
    ...
    if len(mmlu_configs) != 57:
        # In offline or transient-network runs, datasets may return only the
        # aggregate config even though all subject builders are cached.
        cached_root = args.cache / "datasets/cais___mmlu"
        mmlu_configs = tuple(sorted(path.name for path in cached_root.iterdir()
                                    if path.is_dir() and path.name not in {"all", "auxiliary_train"}))
    if len(mmlu_configs) != 57:
