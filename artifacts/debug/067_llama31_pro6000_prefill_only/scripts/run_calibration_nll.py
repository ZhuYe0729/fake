...
from common import CUTLASS, EXPORTER, MODEL, RUN, gpu_list, runtime_env, sha256, write_json

EVALUATOR = Path(__file__).with_name("evaluate_nll.py")
...
def complete(path: Path, policy_hash: str, sample_hash: str, blocks: int) -> bool:
    try:
        ...
        return (len(result["blocks"]) == blocks
                ...
                and runtime.get("checkpoint_config", {}).get("quant_method") == "phase_hetero_mytest"
                and runtime.get("chunked_prefill_enabled") is False
                and runtime.get("phase_trace_events", {}).get("apply_prefill") == blocks * 128
                and runtime.get("phase_trace_events", {}).get("apply_decode", 0) == 0
...
def run_one(row: dict, blocks: int) -> None:
    ...
    samples = RUN / "samples/wikitext_2048_targets.pt"
    ...
    try:
        ...
        subprocess.run(common[:2] + ["--checkpoint", str(temporary)] + common[2:] + ["--phase-hetero"], check=True, env=env)
    finally:
