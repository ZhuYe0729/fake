...
from common import METHODS, RUN, TYPES
...
def llama3_errors() -> dict[tuple[int, str, str], float]:
        ...
        for typ, fused_parts in parts.items():
            for method in METHODS:
                if method == "dense_bf16":
                    ...
                    out[layer, typ, method] = sum(values) / len(values)
    return out
...
def features(policy: dict, errors: dict[tuple[int, str, str], float]) -> torch.Tensor:
    ...
    for name, entry in policy["method_map"].items():
        ...
        tensor[METHODS.index(method), bucket, TYPES.index(typ)] += errors[layer, typ, method]
    return tensor
...
def main() -> None:
    ...
    result = {"model": "llama3.1-8b-instruct", "label_backend": "canonical phase-vLLM direct prompt-logprob", "feature_backend": "067 canonical sparse plus frozen Llama3 non-sparse local-error table", "proxy": "positive normalized local-error aggregation + global/method/bucket/type factors", "fit_split": "p00-p53", "sample_blocks": int(labels["p00"]["sample_count"]), "metrics": summary, "metrics_by_policy_kind": by_kind}
