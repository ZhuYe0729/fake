#!/usr/bin/env python3
"""Measure phase teacher-forced WikiText NLL for one frozen Llama3 policy."""
...
def install(model, policy, phase):
    for method, artifact in STATE.items():
        selected = [name for name, item in policy['method_map'].items() if item[f'{phase}_method'] == method]
...
def main():
    ...
    policy_path = a.policy_json or (ROOT / 'policies/prefill_decode' / f'{a.policy}.json')
    ...
    if a.dense_reference_json and a.dense_reference_json.exists():
        ...
        dense_pre, dense_dec = float(reference['dense_prefill_nll']), float(reference['dense_decode_nll'])
        tokens = int(reference['prefill_tokens'])
    else:
        dense_pre, _ = nll(model, blocks, device, a.batch_size, 'prefill')
        dense_dec, tokens = nll(model, blocks, device, a.batch_size, 'decode')
    install(model, policy, 'prefill'); measured_pre, _ = nll(model, blocks, device, a.batch_size, 'prefill')
    install(model, policy, 'decode'); measured_dec, dec_tokens = nll(model, blocks, device, a.batch_size, 'decode')
    del model; gc.collect(); torch.cuda.empty_cache()
    dp, dd = measured_pre - dense_pre, measured_dec - dense_dec
    row = {'policy_id': a.policy, 'scenario': 'prefill_decode', 'sample_count': len(blocks),
           'dense_prefill_nll': dense_pre, 'dense_decode_nll': dense_dec,
           'prefill_nll': measured_pre, 'decode_nll': measured_dec,
           'delta_prefill_nll': dp, 'delta_decode_nll': dd,
           'target_delta_nll': dp + 80 * dd, 'prefill_tokens': tokens, 'decode_tokens': dec_tokens}
