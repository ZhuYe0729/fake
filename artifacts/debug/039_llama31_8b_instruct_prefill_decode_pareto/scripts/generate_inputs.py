#!/usr/bin/env python3
"""Freeze WikiText blocks and phase-aware Llama3 calibration policies."""
...
def main():
    ...
    directory = ROOT / 'policies' / 'prefill_decode'; directory.mkdir(parents=True, exist_ok=True)
    manifest = []
    for index in range(72):
        policy_id = f'p{index:02d}'
        prefill = method_map(index)
        # Preserve p00--p04 as the five exact two-phase uniform anchors.
        # All remaining calibration policies use a fixed phase permutation.
        decode = prefill if index < len(METHODS) else method_map((index * 17 + 11) % 72)
        for name, item in prefill.items():
            item['decode_method'] = decode[name]['prefill_method']
        policy = {'policy_id': policy_id, 'scenario': 'prefill_decode', 'policy_kind': 'controlled' if index < 37 else 'balanced_mixed', 'default_prefill_method': 'dense_bf16', 'default_decode_method': 'dense_bf16', 'modules_to_not_convert': ['lm_head'], 'method_map': prefill}
