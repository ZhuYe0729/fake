#!/usr/bin/env python3
"""Freeze Llama3.1 prefill-decode architecture, actions, and runner protocol."""
...
def main():
    ...
    rows = []
    for phase, m in (('prefill', 16 * 2048), ('decode', 16)):
        for fused, layer, module_type, n, k in specs:
            query = predictor.predict(m, n, k)
            by_kernel = {x.kernel: x for x in query.candidates}
            conversion = {x.conversion: x for x in predictor.predict_conversion(n, k)}
            for kernel in KERNELS:
                row = {'phase': phase, 'module_name': fused, 'layer': layer, 'module_type': module_type,
                       'm': m, 'n': n, 'k': k, 'kernel': kernel, **candidate_row(by_kernel[kernel])}
                if kernel in CONVERSIONS:
                    conv = conversion.get(CONVERSIONS[kernel])
                    row.update({'conversion': CONVERSIONS[kernel], 'conversion_supported': bool(conv and conv.supported),
                                'conversion_ms': None if conv is None else conv.latency_ms, 'conversion_reason': '' if conv is None else conv.reason})
                else: row.update({'conversion': '', 'conversion_supported': True, 'conversion_ms': 0.0, 'conversion_reason': ''})
                rows.append(row)
    protocol = {'scenario': 'prefill_decode', 'batch': 16, 'input_tokens': 2048,
                'api_output_tokens': 80, 'm_prefill': 32768, 'm_decode': 16,
                'runner': {'enforce_eager': True, 'enable_prefix_caching': False,
                           'enable_chunked_prefill': False, 'gpu_memory_utilization': 0.9,
                           'fresh_process_repetitions': 5, 'timing_scope': 'continuous_phase_heterogeneous_e2e'},
