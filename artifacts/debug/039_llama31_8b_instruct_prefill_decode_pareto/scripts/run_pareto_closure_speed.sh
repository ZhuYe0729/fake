#!/usr/bin/env bash
# Export and measure one screened Pareto policy with the frozen continuous protocol.
set -euo pipefail
if [ "$#" -ne 2 ]; then echo "usage: $0 POINT_ID PHYSICAL_GPU" >&2; exit 2; fi
POINT_ID="$1"
GPU="$2"
...
POLICY="${ROOT}/pareto/policies/${POINT_ID}.json"
CHECKPOINT="${ROOT}/closure/checkpoints/${POINT_ID}"
RUNS="${ROOT}/closure/speed/${POINT_ID}"
mkdir -p "${RUNS}" "${ROOT}/closure/checkpoints" "${ROOT}/logs"
if [ ! -f "${CHECKPOINT}/model.safetensors" ]; then
  CUDA_VISIBLE_DEVICES="${GPU}" "${EXPORT_PYTHON}" "${EXPORT}" --policy-json "${POLICY}" --model-path "${MODEL}" --output-dir "${CHECKPOINT}" --force --prune > "${ROOT}/logs/closure_export_${POINT_ID}.log" 2>&1
fi
if [ ! -s "${RUNS}/measured_0.json" ]; then
  ...
  CUDA_VISIBLE_DEVICES="${GPU}" "${VLLM_PYTHON}" "${BENCH}" --checkpoint "${CHECKPOINT}" --scenario prefill_decode --output-dir "${RUNS}" --gpu-memory-utilization 0.85 --warmup-iters 6 --iters 5 --single-phase main --single-output "${RUNS}/continuous_summary.json" --single-samples-dir "${RUNS}" --reuse-llm > "${ROOT}/logs/closure_speed_${POINT_ID}.log" 2>&1
fi
