...
def main() -> None:
    ...
    args.output_json.write_text(json.dumps({"checkpoint": str(args.checkpoint), "batch": args.batch, "input_seq": args.input_seq, "output_seq": args.output_seq, "elapsed_ms": elapsed_ms, "generated_tokens": generated, "gpu_memory_utilization": args.gpu_memory_utilization, "cuda_visible_devices": os.environ.get("CUDA_VISIBLE_DEVICES"), "cuda_device_name": properties.name, "cuda_device_uuid": str(getattr(properties, "uuid", "unavailable")), "chunked_prefill_enabled": False, "chunked_prefill_guard": "064_process_local_vllm_0.11_compat", "timing_scope": "generate_only_after_loaded_llm", "baseline_alignment": "TokensPrompt,max_model_len,max_num_seqs,prefix_cache_disabled,chunked_prefill_disabled,fixed_greedy_output"}, indent=2) + "\n")
