#!/usr/bin/env python3
"""Machine-readable stage validation for the isolated 064 workflow."""
...
def validate(stage: str) -> dict:
    ...
    if stage in {"bootstrap", "all"}:
        ...
        if len(manifest) != 72 or details["sample_sha256"] != "4c859a5b657834d501ba08b1e212c92dc0a7aec638e9ec67437caf11fc0f52dc":
            errors.append("bootstrap inputs differ from the frozen design")
    if stage in {"isolation", "all"}:
        forbidden = ("artifacts/debug/033_", "artifacts/debug/046_", "artifacts/debug/054_",
                     "artifacts/debug/060_", "artifacts/debug/063_", "artifacts/debug/104_")
        hits = []
    ...
    if stage in {"profile", "all"}:
        ...
        if frozen.get("predictor_root") != expected:
            errors.append("solver did not freeze the 064 predictor")
    ...
    if stage in {"closure", "all"}:
        ...
        for label in sorted(expected_labels):
            ...
            measured = sorted((base / "speed/raw").glob("measured_*.json"))
            ...
            if (len(nll.get("blocks", [])) != 100 or nll.get("runtime", {}).get("chunked_prefill_enabled") is not False
                    ...
                    or any((row.get("batch"), row.get("input_seq"), row.get("output_seq"),
                            row.get("chunked_prefill_enabled")) != (8, 2048, 1, False) for row in raws)):
                incomplete.append(label)
    ...
    if stage in {"tasks", "all"}:
        ...
        for label in selection:
            for task in ("wikitext", "winogrande", "arc_easy", "arc_challenge", "mmlu"):
                ...
                if (guard.get("model_generate_calls", 0) < 1
