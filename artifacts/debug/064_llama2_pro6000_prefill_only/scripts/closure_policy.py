...
def main() -> None:
    ...
    if nll.exists() and all(path.exists() for path in required_speed):
    ...
    try:
        ...
        raw_speed.mkdir(parents=True, exist_ok=True)
        for name in ["warmup"] + [f"measured_{index}" for index in range(args.runs)]:
            target = raw_speed / f"{name}.json"
            if target.exists():
                continue
            subprocess.run([sys.executable, str(HERE / "benchmark_prefill.py"), "--checkpoint", str(checkpoint),
                            ...
                            "--cutlass-wrapper-path", str(CUTLASS), "--output-json", str(target)], check=True, env=env)
        values = [json.loads(path.read_text())["elapsed_ms"] for path in required_speed[1:]]
        mean = statistics.mean(values)
        write_json(output / "speed/summary.json", {"label": args.label, "protocol": PROTOCOL,
                   "measured_elapsed_ms": values, "median_ms": statistics.median(values),
                   "mean_ms": mean, "stdev_ms": statistics.stdev(values) if len(values) > 1 else 0.0,
