...
def main() -> None:
    ...
    blocks = torch.load(EXP / "samples/wikitext_2048_80.pt", map_location="cpu")[:args.blocks]
    ...
    for start in range(0, len(modules), args.module_chunk_size):
        ...
        def hook(name: str):
            def run(_module: torch.nn.Module, inputs: tuple[torch.Tensor, ...], output: torch.Tensor) -> None:
                ...
                if args.phase == "decode" and args.method == "sparse_nvfp4":
                    ...
                    # reported error is cropped back to the 80 real tokens.
        ...
        try:
            with torch.inference_mode():
                for block in blocks:
                    ids = (block[:2048] if args.phase == "prefill" else block).unsqueeze(0).to(device)
    ...
    rows = [{"phase": args.phase, "method": args.method, "layer_bucket": layer_bucket,
             "fused_type": typ, "blocks": args.blocks,
