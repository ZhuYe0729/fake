#!/usr/bin/env python3
"""Freeze Llama2 phase shapes and roofline-predicted legal kernel actions."""
...
def specs(config: AutoConfig) -> list[tuple[str, int, str, int, int]]:
    ...
    return [(f"model.layers.{layer}.{group}.{name}", layer, name, n, k)
            ...
            for group, name, n, k in (("self_attn", "qkv_proj", 3 * hidden, hidden),
...
def main() -> None:
    ...
    protocol = {"scenario": "prefill_decode", "batch": 16, "input_tokens": 2048, "output_tokens": 80,
                "m_prefill": 32768, "m_decode": 16, "methods": list(KERNELS),
                "logical_runtime_mapping": {"marlin_nvfp4": "w4a16_ours"}, "model_path": str(MODEL),
                "module_count": len(specs(config)), "config": {key: int(getattr(config, key)) for key in ("num_hidden_layers", "hidden_size", "intermediate_size", "num_attention_heads")}}
