...
OLD_SPEED = ROOT / "artifacts/debug/035_llama2_prefill_decode_e2e_speed_model/e2e_calibration.csv"
...
def old_e2e_calibrator():
    pairs = sorted((float(row["raw_linear_ms"]), float(row["formal_e2e_ms"])) for row in read(OLD_SPEED))
    xs, ys = [x for x, _ in pairs], pava([y for _, y in pairs])
    def predict(value: float) -> float:
        if value <= xs[0]: return ys[0]
        if value >= xs[-1]: return ys[-1]
        for index in range(1, len(xs)):
            if value <= xs[index]:
                return ys[index - 1] + (value - xs[index - 1]) / (xs[index] - xs[index - 1]) * (ys[index] - ys[index - 1])
...
def main() -> None:
    ...
    model = json.loads((EXP / "reports/quality_coverage_holdout/model.json").read_text())
        ...
        for group, typ in (("self_attn", "qkv_proj"), ("self_attn", "o_proj"), ("mlp", "gate_up_proj"), ("mlp", "down_proj")):
            ...
            for pre in METHODS:
                for dec in METHODS:
                    ...
                    latency = float(pre_action["latency_ms"]) + 80.0 * float(dec_action["latency_ms"])
    ...
    baseline_e2e = calibrate(baseline_raw)
    ...
    rows.append({"policy_id": baseline_id, "quality_budget": 0.0, "predicted_delta_nll": 0.0,
                 "raw_predicted_linear_ms": baseline_raw, "old_calibrated_e2e_ms": baseline_e2e,
    ...
    for budget, state in selections:
        ...
        rows.append({"policy_id": policy_id, "quality_budget": budget, "predicted_delta_nll": sum(value[1] for value in selected),
                     "raw_predicted_linear_ms": raw, "old_calibrated_e2e_ms": calibrate(raw),
    ...
    dense_raw, dense_e2e = baseline_raw, baseline_e2e
    for row in rows:
        row["raw_speedup_vs_dense"] = dense_raw / row["raw_predicted_linear_ms"]
        row["calibrated_speedup_vs_dense"] = dense_e2e / row["old_calibrated_e2e_ms"]
    with (output / "predicted_points.csv").open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(rows[0])); writer.writeheader(); writer.writerows(rows)
    (output / "metadata.json").write_text(json.dumps({"quality": "055 phase-aware canonical real-vLLM ΔNLL proxy",
        "speed": "reused 035 KernelLatencyPredictor roofline + monotone fresh-process E2E calibration",
        "raw_formula": "sum prefill(M=32768) + 80 * sum decode(M=16)", "decode_sparse_nvfp4": "excluded by action-support audit",
