...
from common_prefill_loss import (  # type: ignore
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run sampled sparse BF16 prefill loss policies with one multi-GPU launcher.")
...
def main() -> None:
    ...
    if not policies_csv.exists():
        raise FileNotFoundError(f"Missing policies: {policies_csv}. Run generate_sparse_bf16_policies.py first.")
    ...
    completed = completed_policy_ids(args.output_root, args.output_tag) if args.skip_existing else set()
    ...
    for worker_index, gpu in enumerate(gpus):
        ...
        worker_args.append((args, worker_index, gpu, worker_policies))

    ctx = mp.get_context("spawn")
    with ctx.Pool(processes=len(gpus)) as pool:
        pool.map(run_worker, worker_args)

    output_csv = loss_path(args.output_root, args.output_tag)
    merge_worker_outputs(args.output_root, args.output_tag)
    write_json(
    ...
    print(f"wrote merged loss rows to {output_csv}")


def run_worker(payload: tuple[argparse.Namespace, int, int, list[dict[str, Any]]]) -> None:
    args, worker_index, gpu, policies = payload
    if not policies:
        return
    ...
    output_path = worker_output_path(args.output_root, args.output_tag, worker_index)
    ...
    for policy in policies:
        ...
        print(f"worker={worker_index} gpu={gpu} policy={policy_id} selected={len(selected)}")
        saved = save_original_weights(model, selected)
        try:
            replaced = apply_compressed_weights(model, modules, source_root=args.source_root, method=METHOD, selected_names=selected)
            loss = compute_nll(model, blocks, device=device, batch_size=args.batch_size)
        finally:
            restore_original_weights(model, saved)
            del saved
            cleanup_cuda()
        rows.append(
            {
...
def loss_path(output_root: Path, output_tag: str) -> Path:
    suffix = f"_{output_tag}" if output_tag else ""
    return output_root / "loss" / f"loss_samples_sparse_bf16{suffix}.csv"


def worker_output_path(output_root: Path, output_tag: str, worker_index: int) -> Path:
    suffix = f"_{output_tag}" if output_tag else ""
    return output_root / "loss" / "workers" / f"loss_samples_sparse_bf16{suffix}_worker_{worker_index:02d}.csv"


def completed_policy_ids(output_root: Path, output_tag: str) -> set[str]:
    rows: list[dict[str, Any]] = []
    suffix = f"_{output_tag}" if output_tag else ""
    paths = [loss_path(output_root, output_tag), *sorted((output_root / "loss" / "workers").glob(f"loss_samples_sparse_bf16{suffix}_worker_*.csv"))]
    for path in paths:
...
def merge_worker_outputs(output_root: Path, output_tag: str) -> None:
    rows: list[dict[str, Any]] = []
    suffix = f"_{output_tag}" if output_tag else ""
    for path in sorted((output_root / "loss" / "workers").glob(f"loss_samples_sparse_bf16{suffix}_worker_*.csv")):
        if path.exists() and path.stat().st_size > 0:
            rows.extend(read_csv(path))
    ...
    write_csv(loss_path(output_root, output_tag), merged)
