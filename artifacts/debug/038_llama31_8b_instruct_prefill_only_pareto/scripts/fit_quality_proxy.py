#!/usr/bin/env python3
"""Fit a positive local+global quality proxy and report frozen-holdout metrics."""
...
def error_table():
    ...
    for method in METHODS:
        if method=='dense_bf16':
            ...
            for row in read(ROOT/'local_errors'/f'prefill_{method}.csv'):out[int(row['layer_bucket']),row['fused_type'],method]=float(row['output_rel_mse'])
    return out
def feature(policy,errors):
    x=torch.zeros((5,4,4),dtype=torch.float64)
    for name,item in policy['method_map'].items():
        layer=int(name.split('.')[2]);typ=name.rsplit('.',1)[-1];method=item['prefill_method'];x[METHODS.index(method),layer//8,TYPES.index(typ)]+=errors[layer//8,typ,method]
    return x
def main():
    manifest=json.loads((ROOT/'policies/prefill_only/manifest.json').read_text());labels={x['policy_id']:x for x in read(ROOT/'nll/prefill_only.csv')};errors=error_table();policies=[json.loads(Path(x['path']).read_text()) for x in manifest]
    X=torch.stack([feature(x,errors) for x in policies]);y=torch.tensor([float(labels[x['policy_id']]['target_delta_nll']) for x in policies],dtype=torch.float64);train=torch.tensor([x['split']=='train' for x in manifest]);scale=X[train].sum((1,2,3)).mean().clamp(min=1e-12);X=X/scale
    ...
    result={'model':'positive normalized local-error aggregation + method/bucket/type factors','sample_blocks':int(labels['p00']['sample_count']),'metrics':{s:metrics([x['actual_delta_nll'] for x in rows if x['split']==s],[x['predicted_delta_nll'] for x in rows if x['split']==s]) for s in ('train','holdout')}}
