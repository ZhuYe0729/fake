#!/usr/bin/env python3
"""Restartable phase-unified real-vLLM NLL dispatcher for the 053 policies."""
...
def complete(path: Path, policy_hash: str, sample_hash: str, blocks: int) -> bool:
    try:
        ...
        return len(result["blocks"]) == blocks and runtime["sample_sha256"] == sample_hash and runtime.get("policy_sha256") == policy_hash
...
def one(row: dict, blocks: int) -> None:
    ...
    common = [sys.executable, str(EVALUATOR), "--tokenizer", str(MODEL), "--samples", str(samples), "--output", str(output), "--label", row["policy_id"], "--policy-json", str(policy), "--blocks", str(blocks)]
    if row["policy_id"] == "p00":
        subprocess.run(common[:2] + ["--checkpoint", str(MODEL)] + common[2:], check=True)
        return
    temporary = Path(os.environ.get("COSPAQ_PHASE_TMP_ROOT", "/tmp/cospaq_053_phase")) / row["policy_id"]
    if temporary.exists():
        raise FileExistsError(temporary)
    data = json.loads(policy.read_text())
    methods = [data["default_prefill_method"], data["default_decode_method"]]
    methods.extend(method for pair in data["method_map"].values() for method in pair.values())
    export = [sys.executable, str(EXPORTER), "--model-path", str(MODEL), "--policy-json", str(policy), "--output-dir", str(temporary), "--cutlass-wrapper-path", str(CUTLASS)]
...
def main() -> None:
    ...
    parser.add_argument("--selection", help="comma-separated policy IDs")
    ...
    jobs = [row for row in manifest if row["policy_id"] in selected and not complete(EXPERIMENT / "results" / f"{row['policy_id']}.json", row["sha256"], sample_hash, args.blocks)]
