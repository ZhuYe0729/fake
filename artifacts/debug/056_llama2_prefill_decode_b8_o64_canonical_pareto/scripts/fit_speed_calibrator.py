...
def main() -> None:
    ...
    missing = []
    for item in design:
        run = EXP.parent / "runs" / item["policy_id"] / "iterations.csv"
        values = [float(row["elapsed_ms"]) for row in csv.DictReader(run.open())
                  if row["phase"] == "main" and row["warmup"] == "False"] if run.exists() else []
        if not values:
