...
def main() -> None:
    ...
    predicted = {row["policy_id"]: row for row in csv.DictReader((EXP / "pareto/candidates/predicted_points.csv").open())}
    closure_path = EXP / "pareto/closure_summary.csv"
    closure = {row["policy_id"]: row for row in csv.DictReader(closure_path.open())} if closure_path.exists() else {}
    rows = []
