...
REPO="${COSPAQ_REPO_ROOT:-/home/agent/wja/project/my/cospaq/fake}"
...
# p03 uses sparse-NVFP4 for prefill and dense-NVFP4 for decode.  Reuse the
# already validated vLLM ABI caches from the 056 speed run instead of the
# shared default torch-extension cache, which can stall during profile-run.
if [[ "${CHECKPOINT}" == */speed/runs/p03/checkpoint ]]; then
  export CUTLASS_WRAPPER_SPARSE_NVFP4_EXT_BUILD_DIR="${REPO}/artifacts/debug/056_llama2_prefill_decode_b8_o64_canonical_pareto/build/vllm_sparse_nvfp4_p03"
  export CUTLASS_WRAPPER_NVFP4_EXT_BUILD_DIR="${REPO}/artifacts/debug/056_llama2_prefill_decode_b8_o64_canonical_pareto/build/vllm_nvfp4_p03"
fi
