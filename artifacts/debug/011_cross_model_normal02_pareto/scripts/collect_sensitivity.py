...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect model-level module quality proxy features.")
...
def main() -> None:
    args = parse_args()
    set_model_key(args.model)
    if args.output_root == DEBUG_ROOT:
        args.output_root = DEBUG_ROOT if args.model == "llama2-7b" else DEBUG_ROOT / "models" / args.model
    if args.source_root is None:
        args.source_root = default_model_source_root(args.model)
    ...
    model = load_model_for_quality(device=device, dtype=dtype)
