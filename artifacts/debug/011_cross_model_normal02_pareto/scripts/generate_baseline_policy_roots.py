...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create pseudo-Pareto roots for single-method E2E baselines.")
    parser.add_argument("--model", choices=["llama31-8b"], default="llama31-8b")
...
def main() -> None:
    args = parse_args()
    model_root = args.output_root / "models" / args.model
    rows = read_csv(model_root / "costs" / "module_method_candidates.csv")
    methods = [item.strip() for item in args.methods.split(",") if item.strip()]
    for method in methods:
        root = model_root / "baselines" / method
