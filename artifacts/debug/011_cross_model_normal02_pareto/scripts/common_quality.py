...
def set_model_key(model_key: str) -> None:
    # Keep the copied 007 workflow simple while allowing one script to serve all
    # prepared 003 model roots.
    global MODEL_KEY
    model_spec(model_key)
    MODEL_KEY = model_key


def default_model_source_root(model_key: str, source_root: Path = SOURCE_003_ROOT) -> Path:
    if model_key == "llama2-7b":
        return source_root
    return source_root / "models" / model_key
...
def load_model_for_quality(*, device: str, dtype: torch.dtype) -> nn.Module:
