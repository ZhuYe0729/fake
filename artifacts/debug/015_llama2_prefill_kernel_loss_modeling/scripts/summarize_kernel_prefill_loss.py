...
from common_kernel_prefill_loss import DEBUG_ROOT, f, read_csv, write_csv, write_json
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize kernel-aware prefill local error and loss ablations.")
...
def main() -> None:
    args = parse_args()
    summary_dir = args.summary_dir or args.output_root / "summary" / "kernel_prefill_loss_modeling"
    summary_dir.mkdir(parents=True, exist_ok=True)
    local_rows = read_csv(args.output_root / "sensitivity" / "module_method_kernel_local_errors.csv")
    ...
    write_csv(summary_dir / "layer_type_kernel_local_error_loss_joined.csv", joined)
    write_csv(summary_dir / "kernel_local_error_loss_correlations.csv", correlations)
    ...
    plot_best_proxy(joined, correlations, summary_dir / "best_kernel_proxy_vs_loss_delta.png")
    plot_weight_vs_output(local_rows, summary_dir / "weight_rel_mse_vs_kernel_output_rel_mse.png")
...
def load_ablation_rows(root: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted((root / "ablations").glob("kernel_loss_ablation*.csv")):
...
def join_layer_type(local_rows: list[dict[str, Any]], ablation_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in ablation_rows:
        ...
        item["kernel_module_type"] = local.get("kernel_module_type", "")
...
def plot_layer_depth(rows: list[dict[str, Any]], path: Path) -> None:
    ...
    ax.set_title("Kernel-aware layer-wise compression loss impact")
...
def plot_type_loss(rows: list[dict[str, Any]], path: Path) -> None:
    ...
    ax.set_ylabel("Loss delta vs dense")
    ax.set_title("Kernel-aware linear-type compression loss impact")
...
def plot_best_proxy(rows: list[dict[str, Any]], correlations: list[dict[str, Any]], path: Path) -> None:
    if not rows:
        write_empty_plot(path, "No layer-type rows available for kernel local proxy correlation")
        return
    ...
    ax.set_title(f"Best kernel local proxy vs measured loss delta ({metric})")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(path, dpi=180)
...
def plot_weight_vs_output(rows: list[dict[str, Any]], path: Path) -> None:
    ...
    ax.set_ylabel("kernel output_rel_mse")
    ax.set_title("Weight error vs real-kernel output error")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(path, dpi=180)
