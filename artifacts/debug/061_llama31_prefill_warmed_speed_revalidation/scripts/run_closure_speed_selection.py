#!/usr/bin/env python3
"""Parallel warmed-speed closure for selected solved policies."""
...
DEFAULT = ("point_000", "point_003", "point_005", "point_007", "point_009", "point_011", "point_013", "point_014")
...
def main() -> None:
    ...
    pending = [policy for policy in selected if not (EXP / "pareto/closure/speed" / f"{policy}.json").exists()]
    ...
    def run(policy: str) -> str:
        gpu = available.get()
        try:
            subprocess.run([sys.executable, str(Path(__file__).with_name("run_closure_speed_policy.py")),
