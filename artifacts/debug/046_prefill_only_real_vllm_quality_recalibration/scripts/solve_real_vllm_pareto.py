...
from common import CUTLASS, ROOT, model_root
...
def local_errors() -> dict[tuple[int, str, str], float]:
    rows = read_csv(ROOT / "artifacts/debug/007_llama2_quality_modeling/sensitivity/module_method_errors.csv")
...
def latency_tables() -> dict[str, tuple[dict[str, float], dict[str, float]]]:
    predictor = KernelLatencyPredictor(model_root=DEFAULT_MODEL_ROOT, kernels=("dense_bf16", "dense_nvfp4", "sparse_bf16", "sparse_nvfp4", "marlin_nvfp4"))
    tables = {}
    for fused_type, (n, k) in SHAPES.items():
        ...
        tables[fused_type] = linear, conversion
    return tables


def candidate_groups(coefficients: torch.Tensor, scale: float, errors: dict[tuple[int, str, str], float]) -> list[list[dict[str, object]]]:
    tables = latency_tables()
    groups = []
    for module_index, (name, layer, fused_type) in enumerate(modules()):
        latency, conversion = tables[fused_type]
        rows = []
        for method in METHODS:
            kernel = PREDICTOR_METHOD[method]
            if kernel not in latency:
                continue
            needed = "canonical_to_marlin" if method == "w4a16_ours" else "canonical_to_cutlass" if method == "dense_nvfp4" else None
            if needed is not None and needed not in conversion:
                continue
            quality = errors[layer, fused_type, method] * float(coefficients[METHODS.index(method), layer // 8, TYPES.index(fused_type)]) / scale
            rows.append({"module_index": module_index, "module_name": name, "layer": layer, "module_type": fused_type, "prefill_method": method, "decode_method": method, "prefill_runtime": RUNTIME_METHOD[method], "decode_runtime": RUNTIME_METHOD[method], "quality_cost": quality, "latency_ms": latency[kernel] + (conversion[needed] if needed else 0.0)})
...
def main() -> None:
    ...
    groups = candidate_groups(coefficients, float(quality_model["feature_scale"]), local_errors())
