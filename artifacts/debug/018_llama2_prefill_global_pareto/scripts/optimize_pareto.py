...
from common_pareto import DEBUG_ROOT, METHODS, SCENARIO, f, policy_method_counts, read_csv, sanitize, write_csv, write_json


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Solve quality-constrained per-linear Pareto policies.")
...
def main() -> None:
    ...
    candidate_methods = tuple(method for method in METHODS if any(row.get("method") == method for row in candidates))
    modules = group_candidates(candidates)
    validate_modules(modules, candidate_methods)
...
def validate_modules(modules: dict[str, list[dict[str, Any]]], candidate_methods: tuple[str, ...]) -> None:
    expected = set(candidate_methods)
    bad = []
    for name, rows in modules.items():
        got = {row["method"] for row in rows}
        if got != expected:
            bad.append((name, sorted(got)))
    if bad:
        sample = ", ".join(f"{name}:{methods}" for name, methods in bad[:5])
        raise RuntimeError(f"modules with incomplete candidate sets: {len(bad)}; {sample}")
...
def parse_budgets(spec: str, max_quality: float, max_quality_bins: int, scale: float, max_points: int) -> list[tuple[float, int]]:
    ...
    if max_quality <= 0:
        return [(0.0, 0)]
    # Dense near the conservative end, coarser near the aggressive end.
    ratios = [0.0]
...
def solve_budget(modules: dict[str, list[dict[str, Any]]], budget_int: int, scale: float) -> dict[str, dict[str, Any]]:
    ...
    # dp[q] = (latency, choices tuple of candidate indices)
    ...
    for _module_name, rows in module_items:
        ...
        if not next_dp:
            raise RuntimeError(f"no feasible state at budget={budget}")
...
def prune_states(states: dict[int, tuple[float, tuple[int, ...]]]) -> dict[int, tuple[float, tuple[int, ...]]]:
    # Keep only states that are not dominated by lower/equal quality and lower/equal latency.
...
def summarize_policy(index: int, budget: float, policy: dict[str, dict[str, Any]], modules: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    dense_latency = sum(f(rows_by_method(rows)["dense_bf16"], "latency_cost") for rows in modules.values())
...
def write_policy(output_root: Path, index: int, budget: float, policy: dict[str, dict[str, Any]], point: dict[str, Any]) -> None:
        ...
        modules_json.append(
            {
                ...
                "k": int(f(row, "in_features")),
                "count": 1,
                "selected_prefill_backend": row["method"],
                "selected_decode_backend": row["method"],
                "selected_total_ms": f(row, "latency_cost"),
                "selected_prefill_ms": f(row, "latency_cost"),
    ...
    write_json(
        ...
        {
            "policy_format": "quality_constrained_pareto_prefill_only_v1",
            "scenario": SCENARIO,
            "summary": point,
