...
DEFAULT_DATA_ROOT = Path("/root/data/datasets/flaxquant")
DEFAULT_MODEL_PATH = Path("/root/data/models/Meta-Llama-3.1-8B-Instruct")
DEFAULT_OUTPUT_DIR = Path("outputs/pmpd_style_eval")
DEFAULT_BERTSCORE_MODEL = Path("/root/data/models/bert_score/roberta-large")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-id", default="llama31-8b-instruct")
...
def build_questions(args: argparse.Namespace) -> list[dict[str, Any]]:
    if args.dataset in {"cnn_dm", "cnn_dm_1000"}:
        ...
        fixed_path = Path(__file__).resolve().parents[1] / "inputs/iwslt_question_ids.json"
        fixed_ids = (set(json.loads(fixed_path.read_text())["question_ids"])
                     if args.split == "test" and fixed_path.is_file() else None)
        questions = []
        for i, row in enumerate(dataset):
            translation = row["translation"]
            keep = i in fixed_ids if fixed_ids is not None else len(filter_tokenizer(translation["en"]).input_ids) > 60
            if keep:
                questions.append(
                    {
                        "question_id": i,
