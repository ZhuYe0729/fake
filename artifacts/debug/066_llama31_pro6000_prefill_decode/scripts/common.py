#!/usr/bin/env python3
"""Shared paths, frozen protocol and subprocess helpers for experiment 066."""
...
MODEL = Path(os.environ.get("COSPAQ_MODEL_PATH", "/root/data/models/Meta-Llama-3.1-8B-Instruct")).resolve()
...
PROTOCOL = {
    ...
    "gpu_memory_utilization": float(os.environ.get("COSPAQ_GPU_MEMORY_UTILIZATION", "0.80")),
    "warmups": 1,
    "measured_runs": 5,
}
PROTOCOL["decode_steps"] = PROTOCOL["output_tokens"] - 1
PROTOCOL["teacher_forcing_capacity"] = PROTOCOL["batch"] * (
    PROTOCOL["input_tokens"] + PROTOCOL["output_tokens"])
PMPD = {
...
HISTORICAL_CANONICAL_HASHES = {
    "sparse_bf16": "60aaf02e6246e34735b92d907af48ff1927d3abe6ae33cddb1fb37d1ed3674cf",
    "sparse_nvfp4": "081c0c15479e12a19eeb471104014ca289e167ecfaf39b44fd201f581528ea82",
}
PMPD_DATA_ROOT = Path(os.environ.get("COSPAQ_PMPD_DATA_ROOT", "/root/data/datasets/flaxquant")).resolve()
BERTSCORE_MODEL = Path(os.environ.get("COSPAQ_BERTSCORE_MODEL", "/root/data/models/bert_score/roberta-large")).resolve()
IWSLT_FILTER_TOKENIZER = Path(os.environ.get("COSPAQ_IWSLT_FILTER_TOKENIZER", "/root/data/models/lmsys/vicuna-7b-v1.5")).resolve()
TASK_TEMPLATE = os.environ.get("COSPAQ_TASK_TEMPLATE", "legacy")
