...
from common import CUTLASS, PROTOCOL, VLLM_ROOT, sha256
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input-tokens", type=int, default=PROTOCOL["input_tokens"])
    parser.add_argument("--output-tokens", type=int, default=PROTOCOL["output_tokens"])
    parser.add_argument("--batch-size", type=int, default=PROTOCOL["batch"])
...
def main() -> None:
    ...
    try:
        ...
        for start in range(0, len(samples), args.batch_size):
            ...
            if phase is not None and start + len(batch) < len(samples):
        ...
        runtime = {"backend": "vllm-v1-teacher-forced-decode",
                   ...
                   "max_num_batched_tokens": args.batch_size * required,
                   "expected_max_num_batched_tokens": PROTOCOL["teacher_forcing_capacity"],
                   "gpu_memory_utilization": args.gpu_memory_utilization,
                   "blocks": len(rows), "sample_sha256": sha256(args.samples),
                   "policy_json": str(args.policy_json.resolve()),
                   "policy_sha256": sha256(args.policy_json),
                   "chunked_prefill_enabled": False,
                   "chunked_prefill_guard": "066_process_local_vllm_0.11_compat",
                   "prefix_caching_enabled": False,
                   "sparse_bf16_cache_entries": os.environ.get("CUTLASS_WRAPPER_SPARSE_BF16_MAX_MATMUL_CACHE_ENTRIES", "default-512")}
        if phase is not None:
