#!/usr/bin/env python3
"""Machine-readable stage validation for the isolated 066 workflow."""
...
from common import BUNDLE, PROTOCOL, RESULTS, RUN, TASK_TEMPLATE, VALIDATION, sha256, write_json
...
def validate(stage: str) -> dict:
    ...
    if stage in {"bootstrap", "all"}:
        ...
        if any(not str(Path(row["path"]).resolve()).startswith(str(BUNDLE.resolve())) for row in manifest):
            errors.append("manifest points outside 066")
    if stage in {"isolation", "all"}:
        active = ("common.py", "bootstrap.py", "prepare_canonical_sparse.py", "compression_common.py",
                  "verify_canonical_sparse.py", "export_phase_hetero_model.py",
        ...
        for name in active:
            ...
            for old in ("artifacts/debug/054_", "artifacts/debug/055_", "artifacts/debug/056_",
                        "artifacts/debug/057_", "artifacts/debug/058_", "artifacts/debug/063_",
                        "artifacts/debug/064_", "artifacts/debug/065_"):
                if old in text: forbidden.append(f"{name}:{old}")
        details["isolation"] = {"active_scripts": len(active), "forbidden_references": forbidden,
                                "canonical_source": "066 regenerated Pro 6000 states"}
        errors.extend(forbidden)
    if stage in {"canonical", "all"}:
        provenance = json.loads((RUN / "canonical/provenance.json").read_text())
        ...
        for method, digest in hashes.items():
            if provenance["states"][method]["sha256"] != digest:
                errors.append(f"{method} canonical hash mismatch")
            if verification.get(method, {}).get("checked_linear_weights") != 224:
                errors.append(f"{method} canonical coverage mismatch")
    ...
    if stage in {"tasks", "all"}:
        ...
        details["tasks"] = {"rows": len(summary["rows"]), "expected": expected,
                            "template": summary.get("task_template")}
        ...
        if summary.get("task_template") != TASK_TEMPLATE or TASK_TEMPLATE != "legacy":
            errors.append("task template provenance mismatch")
        for label in summary["selected"]:
            configs = list((RUN / f"tasks/shards/{label}").glob("**/run_config.json"))
            if not configs or any(json.loads(path.read_text()).get("task_template") != "legacy"
                                  for path in configs):
                errors.append(f"missing legacy shard provenance: {label}")
