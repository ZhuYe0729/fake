...
def validate_backend_pairs(point_index: int, pair_counts: Counter[tuple[str, str]]) -> None:
    if point_index == 9:
        expected = Counter({("marlin_nvfp4", "marlin_nvfp4"): 128, ("dense_nvfp4", "marlin_nvfp4"): 96})
        if pair_counts != expected:
            raise RuntimeError(f"point_009 backend pairs mismatch: got={dict(pair_counts)} expected={dict(expected)}")
